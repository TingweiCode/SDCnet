...
net = torch.nn.DataParallel(UNet())
torch.backends.cudnn.benchmark = True
net.load_state_dict(torch.load('Unet_model.pth'),False)
net.cuda()
net.eval()
...
with torch.no_grad():
    for i in tqdm(range(len(datas))):
        ...
        plt.savefig('./test_obs_photo_Unet/predicted_%d.png' %i,dpi = 600)
        ...
        plt.xlabel('Period(s)',font2)
        plt.ylabel('Group Velocity(km/s)',font2)
        plt.savefig('./test_obs_photo_Unet/label_%d.png' %i,dpi = 600)
        ...
        plt.xlabel('Period(s)',font2)
        plt.ylabel('Group Velocity(km/s)',font2)
        plt.savefig('./test_obs_photo_Unet/data_%d.png' %i,dpi = 600)
